...
public class RBridge {
	...
	public TestResult testRepeatedMeasuresOneWayANOVA(double[][] values) {
		...
		try {
			...
			// Extract TestResult manually, because result data does not conform
			// to any common class
			...
			/*
			 * // Initialise Ez and Reshape libraries. Load package from CRAN
			 * mirror, if not available yet. engine.eval("usePackage('ez')");
			 * engine.eval("usePackage('reshape')");
			 * 
			 * // Pass array and 2d-array dimensions to R engine.assign("v", v);
			 * engine.assign("dimensions", new
			 * int[]{values.length,values[0].length});
			 * 
			 * // Add column with fold indices
			 * engine.eval("folds<-c(1:dimensions[2])");
			 * engine.eval("v<-c(folds,v)");
			 * engine.eval("dimensions[1]<-dimensions[1]+1");
			 * 
			 * // Transform it into a matrix in R, then transform the matrix
			 * into a data frame
			 * engine.eval("m<-matrix(v,nrow = dimensions[2],ncol = dimensions[1])"
			 * ); engine.eval("df <- as.data.frame(m)");
			 * engine.eval("names(df)<-c('Fold',1:(dimensions[1]-1))");
			 ...
			 * engine.eval("names(df)<-c('Fold','Model','Performance')");
			 * engine.
			 * eval("df$Fold <- factor(df$Fold, labels = c(1:(dimensions[2]))");
			 * 
			 * // Create ANOVA-model engine.eval(
			 * "model <- ezANOVA(data=df, dv=.(Performance), wid=.(Fold),within=.(Model), detailed=TRUE, type=3)"
			 * );
			 * 
			 * // Fix names to ease extraction of data engine.eval(
			 * "names(model)<-c('ANOVA','Mauchlys','SphericityCorrections')");
			 * 
			 * // Extract TestResult manually, because result data does not
			 * conform to any common class double p =
			 * getDoubleOrNaN(engine.eval("get('ANOVA', model)$p[2]")); double
			 * pMauchly =
			 * getDoubleOrNaN(engine.eval("get('Mauchlys', model)$p")); double
			 * pCorrectedGreenhouseGeissner = getDoubleOrNaN(engine.eval(
			 * "get('p[GG]',get('SphericityCorrections',model))")); double
			 * pCorrectedHuynhFeldt = getDoubleOrNaN(engine.eval(
			 * "get('p[HF]',get('SphericityCorrections',model))")); double
			 * statistic =
			 * getDoubleOrNaN(engine.eval("get('ANOVA', model)$F[2]")); double
			 * df = getDoubleOrNaN(engine.eval("get('ANOVA',model)$DFn[2]"));
			 * String method = "Repeated Measures One-Way ANOVA";
			 * HashMap<String,Double> parameter = new HashMap<String,Double>();
			 * parameter.put("df", df);
			 * 
			 * HashMap<String,Double> sphericityParameters = new
			 * HashMap<String,Double>();
			 * sphericityParameters.put("p_{GG}",pCorrectedGreenhouseGeissner);
			 * sphericityParameters.put("p_{HF}",pCorrectedHuynhFeldt);
			 * TestResult testSphericity = new TestResult("Mauchly's test",
			 * sphericityParameters, pMauchly, 0);
			 * testSphericity.setStatisticType("\\sigma^2");
			 * 
			 * result=new TestResult(method,parameter,p,statistic);
			 * result.getAssumptions().put("Sphericity", testSphericity);
	...
	public PairwiseTestResult testDunett(double[][] values) {
		...
		/*
		 * double a[] = values[0]; values[0] = values[values.length-1];
	...
	public double[][] adjustP(PairwiseTestResult result, String method) {
		...
		// Determine whether the p-values represent comparisons against a
		// control method
		// If this is the case, extract the first column. Otherwise, use the
		// whole matrix for adjustment.
		/*
		 * int dimX,dimY; if(result.isBaselineEvaluation()){ v=new
		 * double[result.getpValue()[0].length]; for(int i=0; i<v.length; i++){
		 * v[i]=result.getpValue()[i][0]; } }else{
		 * v=flattenArray(result.getpValue()); dimX=result.getpValue().length;
