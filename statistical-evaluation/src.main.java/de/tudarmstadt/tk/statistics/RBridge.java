 ...
 * 
 * Requirements:
 * 1) R must be installed (Download from http://cran.r-mirror.de/)
 * 2) Package "RJava" must be installed. Type into R console "install.package(RJava)".
 * 
 * EITHER Command line in UNIX (store in .bashrc):
 * 1) Add JARs JRIEngine.jar, JRI.jar, REngine.jar to CLASSPATH. Example: export CLASSPATH=.:/usr/lib/R/site-library/rJava/jri/
 * 2) Set R_HOME path. Example: export R_HOME=/usr/lib/R. Retrieve path by executing "R.home(component = "home")" in R
 * 3) Add native JRI-library to LD_LIBRARY_PATH. It should be located within the same directory as the JAR's. Example: export LD_LIBRARY_PATH=/usr/lib/R/site-library/rJava/jri/
 * 
 * OR Eclipse:
 * 1) Add JARs to Project > Properties > Java Build Path > Libraries
 * 2) Add JRI Library path to Run > Run Configurations > VM Arguments. Example: -Djava.library.path=.:/usr/lib/R/site-library/rJava/jri/
 * 3) Add the R_HOME variable to Run > Run Configurations > Environment. Example: New variable R_HOME with value /usr/lib/R
 *
 * Tutorial on using R from Java: http://blog.comsysto.com/2013/07/10/java-r-integration-with-jri-for-on-demand-predictions/
 *
 */
public class RBridge{
    ...
    /**
     * (Internal) Create singleton instance of class RStatistics.
     * Main goal: Create R-engine instance to be used for any of the statistics methods. Multiple calls share a common context which is maintained throughout the lifecycle of a Rengine instance. 
     * @param chatty If true, a event listener is attached to the R session to track events and errors and print them to the log
     */
    private RBridge(boolean chatty) {
		...
		//If "chatty"-flag set, attach listener to R engine to track events and errors
		...
		//Initialise R Engine. 
		//First parameter: parameter list comprising --vanilla. Crucial to initiate a clean R session
		//Second parameter enables/deactivates R main loop. In this case, we only want to use R as calculation slave, thus deactivated it.
		//Third parameter used to attach listener and listen to R events, e.g. errors
        ...
        try{
        	...
        	//Add function to R session to install packages only if not already present. Use first specified library directory to store packages.
    ...
    /**
     * Create singleton instance of class RStatistics.
     * Main goal: Create R-engine instance to be used for any of the statistics methods. Multiple calls share a common context which is maintained throughout the lifecycle of a Rengine instance. 
     * @param chatty If true, a event listener is attached to the R session to track events and errors and print them to the log
     * @return An instance of class RStatistics
     */
    public static RBridge getInstance(boolean chatty) {
        if (instance == null) {
            synchronized (RBridge.class) {
                if (instance == null) {
                    instance = new RBridge(chatty);
                }
            }
        }
        return instance;
    ...
    /**
     * Transforms either a htest or pairwise.htest R object to its java equivalent TestResult and PairwiseTestResult
     * @param The name of the R resource that represents the htest or pairwise.htest object
     * @return An instance of class AbstractTestResult that can be casted to either TestResult or PairwiseTestResult
     */
    ...
    /**
     * Wrapper method to perform the Kruskal-Wallis test in R.
     * The Kruskal-Wallis test is a non-parametric alternative to ANOVA. It tests whether >2 samples originate from the same distribution. Null hypothesis: there is no entity in the group for which the performance is significantly different from the others.
     * @param values Two-dimensional double array with >2 samples of performance measures. E.g. columns=different models, rows=F-measures from k-fold-cross-validation. Samples of unequal size must be filled with NaNs!
	 * @return Instance of class TestResult, comprising p-Values, statistics, the method applied, etc. and a method for human-friendly printing
     */
    public TestResult testKruskalWallis(double[][] values){
		...
		try{			
	        ...
	        // Transform it into a matrix in R, as required by the following methods
	        engine.eval("m<-matrix(v,nrow = dimensions[2],ncol = dimensions[1])");
	        
	        // Transform matrix into list, because the KW-test requires a list if no factors are specified
	...
	/**
	 * Wrapper method to perform the Friedman test in R.
	 * The Friedman test is a non-parametric statistical test. Similar to the parametric repeated measures ANOVA, it is used to detect differences in treatments across multiple test attempts. Null hypothesis: there is no entity in the group for which the performance is significantly different from the others.
	 * @param values Individual performance results of different items (e.g. models -> rows) over different domains or folds (columns). Please ensure that data is measured on at least two domains/folds!
	 * @return Instance of class TestResult, comprising p-Values, statistics, the method applied, etc. and a method for human-friendly printing
	 */
	public TestResult testFriedman(double[][] values){
		...
		try{			
	        ...
	        // Transform it into a matrix in R, as required by the followin"The Friedman test is only suitable for repeated measured. Please provide appropriate input. Returning null."g methods
	        engine.eval("m<-matrix(v,nrow = dimensions[2],ncol = dimensions[1])");
	...
	/**
	 * Wrapper method to perform the Nemenyi test in R
	 * The Nemenyi test is a post-hoc test intended to find the groups of data that differ after a statistical test of multiple comparisons (such as the Friedman test) has rejected the null hypothesis. The test makes pair-wise tests of performance. Null hypothesis: the performance of the pairwise entities is similar.
	 * @param values Individual performance results of different items (e.g. models -> rows) over different domains (columns)
	 * @return Instance of class PairwiseTestResult, comprising pairwise p-Values, statistics, the method applied, etc. and a method for human-friendly printing
	 */
	public PairwiseTestResult testNemenyi(double[][] values){
		...
		try{
	        ...
	        // Initialise Nemenyi library. Load package from CRAN mirror, if required.
	        engine.eval("usePackage('PMCMR')");
	        
	        // Perform test on matrix (second parameter determines to how many digits the samples are rounded) 
	...
	/**
	 * Wrapper method to perform McNemar's test in R
	 * McNemar's test is a non-parametric alternative to the t-test. The null hypothesis of marginal homogeneity states that the two marginal probabilities for each outcome are the same.
	 * @param contingencies: A two-dimensional contingency table which comprises the outcomes of two tests on a sample of n subjects (Test 1 positive/negative vs. Test 2 positive/negative). Entries should be nonnegative integers. 
	 * @return Instance of class TestResult, comprising p-Values, statistics, the method applied, etc. and a method for human-friendly printing
	 */
	...
	/**
	 * Wrapper method to perform the Mann-Whitney-U (equivalent to Mann–Whitney–Wilcoxon, Wilcoxon rank-sum, or Wilcoxon–Mann–Whitney) test in R
	 * A non-parametric alternative to the t-test. Null hypothesis: two populations are the same. Alternative hypothesis: a particular population tends to have larger values than the other.
	 * @param x: First sample of independent performance measures
	 * @param y: Second sample of independent performance measures
	 * @return Instance of class TestResult, comprising p-Values, statistics, the method applied, etc. and a method for human-friendly printing
	 */
	...
	/**
	 * Wrapper method to perform the Wilcoxon Signed-Rank test in R
	 * A non-parametric alternative to the t-test for matched pairs. Null hypothesis: two populations have the same mean ranks. Alternative hypothesis: One population differs in its mean rank from the other.
	 * @param x: First sample of dependent performance measures
	 * @param y: Second sample of dependent performance measures
	 * @return Instance of class TestResult, comprising p-Values, statistics, the method applied, etc. and a method for human-friendly printing
	 */
	...
	/**
	 * Wrapper method to perform the Wilcoxon Signed-Rank test in R for pairwise comparison of >2 groups.
	 * @param values Individual performance results of different items (e.g. models -> rows) over different domains or folds (columns).
	 * @param isBaselineEvaluation If true, only compare each group against the first.
	 * @return Instance of class {@link PairwiseTestResult}, comprising pairwise p-Values, statistics, the method applied, etc. and a method for human-friendly printing
	 */
	...
	/**
	 * Wrapper method to perform the pairwise t-Test in R
	 * Corresponds to performing multiple t-tests to compare the means of pairwise samples. Null hypothesis: there are no statistically significant differences in the means of a pairwise sample. 
	 * @param values Individual performance results of different items (e.g. models -> rows) over different domains or folds (columns). Please ensure that data is measured on at least two domains/folds!
	 * @param paired Specified whether individual samples should be treated as paired (independent/dependent)
	 * @return Instance of class {@link PairwiseTestResult}, comprising pairwise p-Values, statistics, the method applied, etc. and a method for human-friendly printing
	 */
	public PairwiseTestResult testPairwiseT(double[][] values, boolean paired){
		...
		try{
	        ...
	        // Transform data frame to long format and set models as grouping factor
	        ...
	        // Perform pairwise t-test without corrections (these can be applied later)
	...
	/**
	 * Wrapper method to perform the dependent t-test (paired-samples t-test) in R
	 * A parametric test which compares the means of two related groups to detect whether there are any statistically significant differences between these means. Null hypothesis: there are no statistically significant differences in means. 
	 * @param x: First sample of dependent performance measures
	 * @param y: Second sample of dependent performance measures
	 * @return Instance of class TestResult, comprising p-Values, statistics, the method applied, etc. and a method for human-friendly printing
	 */
	public TestResult testDependentT(double[] x, double[] y){
		...
		try{
			...
			//Check if assumptions for this parametric test (normality of differences) are met and store result in parent test
	...
	/**
	 * Wrapper method to perform the Repeated-Measures One-Way ANOVA in R
	 * A parametric test generalizing the dependent t-test to more than two groups. Null hypothesis: there is no entity in the group for which the performance is significantly different from the others.
	 * @param values Individual performance results of different items (e.g. models -> rows) over different domains or folds (columns). Please ensure that data is measured on at least two domains/folds!
	 * @return Instance of class {@link TestResult}, comprising p-Values, statistics, the method applied, etc. and a method for human-friendly printing. The corrected p-values according to Greenhouse-Geissner and Huynh-Feldt are comprised in the parameters map.
	 */
	...
	/**
	 * Wrapper method to perform Dunett's test after a Repeated-Measures One-Way ANOVA in R
	 * A parametric post-hoc test after ANOVA for comparing different items against the first (the baseline).
	 * @param values Individual performance results of different items (e.g. models -> rows) over different domains or folds (columns). The first item represents the baseline to be evaluated against. Please ensure that data is measured on at least two domains/folds!
	 * @return Instance of class {@link PairwiseTestResult}, comprising p-Values, statistics, the method applied, etc. and a method for human-friendly printing.
	 */
	...
	/**
	 * Wrapper method to perform Tukey's test after a Repeated-Measures One-Way ANOVA in R
	 * A parametric post-hoc test after ANOVA for pairwise n:n comparisons.
	 * @param values Individual performance results of different items (e.g. models -> rows) over different domains or folds (columns). Please ensure that data is measured on at least two domains/folds!
	 * @return Instance of class {@link PairwiseTestResult}, comprising p-Values, statistics, the method applied, etc. and a method for human-friendly printing.
	 */
	 ...
	 * @param result The resulting PairwiseTestResult from a multiple comparisons test
	 * @param method The method to be applied e.g. "bonferroni" for a conservative adjustment
	...
	/**
	 * Wrapper method to create a qqplot in R, comparing the given samples to a normal distribution
	 * @param samples The samples to compare against a normal
	 * @param filename The path where the plot should be stored
	 * @param model The name of the model that generated the samples
	 * @param measure The name of the performance measure the samples represent
	...
	/**
	 * Wrapper method to create a Box-whisker diagram in R, comparing samples for different models
	 * @param s Individual performance results of different items (e.g. models -> rows) over different domains or folds (columns). Please ensure that data is measured on at least two domains/folds!
	 * @param filename The path where the plot should be stored
	 * @param measure The name of the performance measure the samples represent
	 * @return True if plotting and saving to file succeeded, false otherwise. 
	 **/
	 ...
	 * @param edgelist a 2-dimensional integer array representing edges. Each subarray contains two numbers, indicating start- and endpoint of one edge.
	 * @param filename The place where the plot should be stored
	 ...
	 * @param inputArray a 2-dimensional double array
	 * @return a one-dimensional double array comprising all values from the 2-dimensional input array
	 */
	 ...
	 * @param inputArray a 2-dimensional integer array
	 * @return a one-dimensional integer array comprising all values from the 2-dimensional input array
	 */
	...
	/**
	 * Listener class to transfer the console output of the R environment to Java
